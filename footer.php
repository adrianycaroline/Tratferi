<?php 

?>

<!DOCTYPE html>
<html lang="Pt_BR">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="css/bootstrap.min.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.3/font/bootstrap-icons.css">
    <link rel="stylesheet" href="CSS/estilo.css">
    <title>Footer</title>
</head>
<body>
    <div class="" style="background-color: #1977ad;" >
        <figure>
          <blockquote class="blockquote">
              <div class="d-flex justify-content-around">
                    <div style="color: white;">Atendimento <i class="bi bi-headset" style="color: white;"></i> <br>
                      <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-envelope-fill" viewBox="0 0 16 16">
                          <path d="M.05 3.555A2 2 0 0 1 2 2h12a2 2 0 0 1 1.95 1.555L8 8.414.05 3.555ZM0 4.697v7.104l5.803-3.558L0 4.697ZM6.761 8.83l-6.57 4.027A2 2 0 0 0 2 14h12a2 2 0 0 0 1.808-1.144l-6.57-4.027L8 9.586l-1.239-.757Zm3.436-.586L16 11.801V4.697l-5.803 3.546Z"/>
                        </svg>
                        <a href="mailto:tratferi_site@outlook.com?subject=Contato do Site" style="color: #fff;">
                        &nbsp;tratferi_site@outlook.com
                        </a>
                      </div>
                      <br>
                      <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-telephone-fill" viewBox="0 0 16 16">
                          <path fill-rule="evenodd" d="M1.885.511a1.745 1.745 0 0 1 2.61.163L6.29 2.98c.329.423.445.974.315 1.494l-.547 2.19a.678.678 0 0 0 .178.643l2.457 2.457a.678.678 0 0 0 .644.178l2.189-.547a1.745 1.745 0 0 1 1.494.315l2.306 1.794c.829.645.905 1.87.163 2.611l-1.034 1.034c-.74.74-1.846 1.065-2.877.702a18.634 18.634 0 0 1-7.01-4.42 18.634 18.634 0 0 1-4.42-7.009c-.362-1.03-.037-2.137.703-2.877L1.885.511z"/>
                        </svg>
                        <a href="https://api.whatsapp.com/send?phone=5511946990301&text=Ol%C3%A1" style="color: #fff;">
                        &nbsp;(11) 94699-0301
                        </a>
                      </div>
                    </div>
                      <div style="color: white;">
                        <img src="images/logoPesq.png" class="img-responsive" width="170">
                      </div>
                      <div>
                        <div class="">
                          <table class="">
                            <tr class="nav-item">
                              <a class="nav-link" href="index.php#" style="color: #fff; font-size: 20px;">Home</a>
                            </tr>
                            <tr class="nav-item">
                              <a class="nav-link" href="conscientização.php" style="color: #fff; font-size: 20px;">Conscientização</a>
                            </tr>
                            <tr class="nav-item">
                              <a class="nav-link" href="download.php" style="color: #fff; font-size: 20px;">Downloads</a>
                            </tr>
                            <tr class="nav-item">
                              <a class="nav-link" href="sobre.php" style="color: #fff; font-size: 20px;">Sobre</a>
                            </tr>
                            <tr class="nav-item">
                              <a class="nav-link " href="contato.php" id="polit">Fale conosco</a>
                            </tr>
                            <tr>
                              <a href="Politica_Privacidade.php" id="polit2"> Política de Privacidade </a>
                            </tr>
                          </table>
                        </div>
                      </div>
                  </div>
          </blockquote>
              <!-- <figcaption class="blockquote-footer">
              </figcaption> -->
        </figure>
        <div class="text-bg-info p-3" style="background: -webkit-gradient(linear, left top, right top, from(#38B6FF), to(#38B6FF)) no-repeat;">
          <h6 class="text-center" style="color: white">©2023 Tratferi-Todos os direitos reservados</h6></div>
        </div>
      </div>
</body>
    <script type="module" src="https://unpkg.com/ionicons@5.5.2/dist/ionicons/ionicons.esm.js"></script>
    <script nomodule src="https://unpkg.com/ionicons@5.5.2/dist/ionicons/ionicons.js"></script>
</html>